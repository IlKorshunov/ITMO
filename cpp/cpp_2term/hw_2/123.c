	...
	if (image->type_color == 3)
	{
		for (int i = 0; i < image->image_height; ++i)
		{
			for (int j = 0; j < image->image_width; ++j)
			{
				if (image->type == P6)
				{
					image->outPalet[i * image->image_width * 3 + 3 * j] = image->PLTE_array[3 * image->data[i * image->image_width + j]];
					image->outPalet[i * image->image_width * 3 + 3 * j + 1] = image->PLTE_array[3 * image->data[i * image->image_width + j] + 1];
					image->outPalet[i * image->image_width * 3 + 3 * j + 2] = image->PLTE_array[3 * image->data[i * image->image_width + j] + 2];
				}
				else
				{
					image->outPalet[i * image->image_width * 3 + 3 * j] = image->PLTE_array[3 * image->data[i * image->image_width + j]];
