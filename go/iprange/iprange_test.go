...
func FuzzParseList(f *testing.F) {
	testcases := []string{
		"192.168.10-20.1", "0-255.1.1.1",
		"192.168.1.1, 192.168.1.1/24, 192.168.1.*, 192.168.1.10-20",
		"192.168.1.10",
		"192.168.1.10, 192.168.1.1-20, 192.168.1.10/29",
	}
	for _, tc := range testcases {
		f.Add(tc)
	}
	f.Fuzz(func(t *testing.T, orig string) {
		listActual, err := ParseList(orig)
		listIp := strings.Split(orig, ",")
		myResult := make([][2]string, len(listIp))
		for count, _ := range listIp {
			myResult[count] = MyParseString(listIp[count])
		}
		if err != nil {
			t.Fatalf("Error parsing list: %v", err)
		}
		for count, r := range listActual {
			if r.Min.To4() == nil || r.Max.To4() == nil || r.Min.To16() == nil {
				t.Errorf("Invalid IP range: min=%v, max=%v", r.Min, r.Max)
				continue
			}
			if bytes.Compare(r.Min, r.Max) > 0 {
				t.Errorf("Min IP is greater than Max IP: min=%v, max=%v", r.Min, r.Max)
			}
			min := r.Min.String()
			max := r.Max.String()
			if min != myResult[count][0]|| max != myResult[count][1] {
				t.Errorf("wrong ans for example: %s. Expected min: %s, max: %s , Actual min: %s, max: %s ", listIp[count],
				 myResult[count][0], myResult[count][1], r.Min, r.Max)
			}
		}
	})
}
